        ...
        return (
            ...
            >
                <ul className='flex flex-col  py-2'>
                    {
                        fillterResult && fillterResult.map((searchCountry, index) => (
                            <Link
                                ref={index === focusedIndex ? this.divRef : null}
                                to={`countrydetails/${searchCountry.name.common}`} key={searchCountry.name.common}
                                className={`px-2  hover:bg-slate-200 py-2 flex items-center justify-between text-[0.8rem] ${index === focusedIndex ? 'bg-slate-200' : ''}`}>
                                <li >{searchCountry.name.common}</li>
                                <figure className='max-w-[3rem] h-[1.8rem]'>
                                    <img src={searchCountry.flags.png} alt="" className='h-[100%] object-cover' />
                                </figure>
                            </Link>
                        )
