...
class CountryDetails extends Component {
  ...
  render() {
    ...
    const countryCurrency =
      countryDetails && Object.entries(countryDetails.currencies);
    return (
      <section className="countrydetails_container mt-5 h-[85vh] flex flex-col gap-y-9 ">
        ...
        <div className="goto_previouspage flex">
          <Link
            to="/"
            className="back_page capitalize flex items-center bg-[#ffffff] px-6 py-1 shadow rounded-md gap-x-1"
          >
        ...
        {countryDetails && (
          <div className="flex ">
            <aside className="country_logo w-[30%] overflow-y-hidden">
              <figure
                className={`h-[15rem] animated ${animation && "fade-in-top"}`}
              >
                <img
                  src={countryDetails.flags.png}
                  alt={countryDetails.flags.alt}
                  className="w-[100%] h-[100%] aspect-square object-cover"
                />
              </figure>
            </aside>
            <aside
              className={`country_details w-[70%]  px-20  overflow-hidden capitalize font-light`}
            >
              <div
                className={`flex flex-col animated ${
                  animation && "fade-in-right"
                } h-[100%] justify-between`}
              >
                <header>
                    ...
                    <ul>
                      <li>
                        ...
                        {Object.entries(countryDetails.name.nativeName).map(
                          (natName) => (
                            <span className="ms-1 text-[#535354]">
                              {natName[1].official}
                            </span>
                          )
                        )}
                    ...
                    <ul>
                      ...
                      <li>
                        ...
                        {Object.entries(countryDetails?.languages).map(
                          (lang) => {
                            return (
                              <span className="ms-1 text-[#535354]" key={lang}>
                                {lang[1]},{" "}
                              </span>
                            );
                          }
                ...
                <div className="borders flex items-center gap-x-1 ">
                  ...
                  <div className="text-[#535354] flex  justify-center overflow-x-auto web">
                    {countryDetails?.borders?.map((border) => (
                      <Link
                        className="block bg-[#ffffff] shadow px-8"
                        key={border}
                      >
                        {border}
                      </Link>
