...
class SearchNavbar extends Component {
  ...
  handleKeyPressDown = (e) => {
    ...
    if (key === "ArrowDown") {
      nextIndexCount = (focusedIndex + 1) % fillterResult.length;
      e.preventDefault();
      e.stopPropagation();
    }
    if (key === "ArrowUp") {
      nextIndexCount =
        (focusedIndex + fillterResult.length - 1) % fillterResult.length;
    }
    ...
    return (
      <nav className="search_country_container flex mt-9 justify-between items-center sticky top-[4.5rem] z-10 ">
        ...
        >
          ...
          {fillterResult != "" && (
            <Search fillterResult={fillterResult} focusedIndex={focusedIndex} />
          )}
        </div>
        <div className="filter_by_region relative bg-[#ffffff] shadow-sm py-2 px-3 text-[0.8rem] rounded flex">
          <header className="flex items-center gap-x-1">
            ...
            <figure
              className="max-w-[1.2rem] cursor-pointer"
              onClick={() =>
          ...
          <div
            className={`${
              dropDownOpen ? "max-h-[10rem]" : "max-h-0"
            } bg-[#ffffff] absolute left-0 top-11 right-0 shadow-sm rounded overflow-y-hidden transition-[max-height] duration-500`}
          >
              ...
              {country.map((coun) => {
                return (
                  <Link to={`/region/${coun}`} key={coun}>
                    <li
                      className="cursor-pointer px-4 py-1 hover:bg-slate-200"
                      aria-label={coun}
                    >
                      {coun}
                    </li>
